...
public class CodeFirstRestTemplateSpringmvc extends CodeFirstRestTemplate {
    ...
    private void testIntf() {
        ...
        ResponseEntity<Person> responseEntity = intf.responseEntity(date);
        TestMgr.check(date, responseEntity.getBody());
        TestMgr.check(true, responseEntity.getHeaders().getFirst("h1").contains("x-cse-src-microservice=springmvcClient"));
        TestMgr.check(true, responseEntity.getHeaders().getFirst("h2").contains("x-cse-src-microservice=springmvcClient"));
        ...
        TestMgr.check(true, ((String)cseResponse.getHeaders().getFirst("h1")).contains("x-cse-src-microservice=springmvcClient"));
        TestMgr.check(true, ((String)cseResponse.getHeaders().getFirst("h2")).contains("x-cse-src-microservice=springmvcClient"));
    }
